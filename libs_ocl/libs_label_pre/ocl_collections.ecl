...
%    and 0 otherwise
%hanhdd: update for oclUndefined

delay ocl_col_excludesAll(X, _, _) if var(X).%nonground(X).
delay ocl_col_excludesAll(_, Y, _) if var(Y).%nonground(Y).
ocl_col_excludesAll(Col1, Col2, Result) :-
  ( Col1 = oclUndefined 
    -> Result = oclUndefined; 
    ( Col2 = oclUndefined 
...
ocl_col_isEmpty(oclUndefined, oclUndefined).
ocl_col_isEmpty([], 1) :- !.
...
ocl_col_notEmpty(oclUndefined, oclUndefined).
ocl_col_notEmpty([], 0) :- !.
...
ocl_col_product([], _, []) :- !.
ocl_col_product(_, [], []) :- !.
ocl_col_product(oclUndefined, _, oclUndefined).
ocl_col_product(_, oclUndefined, oclUndefined).
...
ocl_set_unionSet(oclUndefined, _, oclUndefined). 
ocl_set_unionSet(_, oclUndefined, oclUndefined). 
...
ocl_set_unionBag(oclUndefined, _, oclUndefined). 
ocl_set_unionBag(_, oclUndefined, oclUndefined). 
...
ocl_set_equals(oclUndefined, _, oclUndefined). 
ocl_set_equals(_, oclUndefined, oclUndefined). 
ocl_set_equals(Set1, Set2, Result) :- 
   list_to_ord_set(Set1, Aux1),
   list_to_ord_set(Set2, Aux2),
...
ocl_set_intersectionSet(oclUndefined, _, oclUndefined). 
ocl_set_intersectionSet(_, oclUndefined, oclUndefined). 
...
ocl_set_intersectionBag(oclUndefined, _, oclUndefined). 
ocl_set_intersectionBag(_, oclUndefined, oclUndefined). 
...
ocl_set_difference(oclUndefined, _, oclUndefined). 
ocl_set_difference(_, oclUndefined, oclUndefined). 
ocl_set_difference(Set1, Set2, Result):- 
   list_to_ord_set(Set1, Aux1),
   list_to_ord_set(Set2, Aux2),
...
ocl_set_excluding(oclUndefined, _, oclUndefined ).
ocl_set_excluding(_, oclUndefined, oclUndefined ).
ocl_set_excluding(Set, Obj, Result ):- 
   ocl_mustBeObject(Obj, Object),
   list_to_ord_set(Set, Aux),
...
ocl_set_symmetricDifference(oclUndefined, _, oclUndefined ).
ocl_set_symmetricDifference(_, oclUndefined, oclUndefined ).
ocl_set_symmetricDifference(Set1, Set2, Result) :-
   list_to_ord_set(Set1, Aux1),
   list_to_ord_set(Set2, Aux2),
...
ocl_set_flatten(oclUndefined, oclUndefined). 
ocl_set_flatten(Set, Result) :- 
   flatten(Set, Result).
...
ocl_set_asSet(oclUndefined, oclUndefined).
ocl_set_asSet(Set, Result) :- 
...
ocl_set_asOrderedSet(oclUndefined, oclUndefined). 
ocl_set_asOrderedSet(Set, Result) :- 
...
ocl_set_asSequence(oclUndefined, oclUndefined).
ocl_set_asSequence(Set, Result) :- 
   ( is_list(Set) -> Result = Set; Result = [Set]).
...
ocl_set_asBag(oclUndefined, oclUndefined).
ocl_set_asBag(Set, Result) :- 
   ( is_list(Set) -> Result = Set; Result = [Set]).
...
ocl_bag_equals([], [], 1).
ocl_bag_equals(oclUndefined, _, oclUndefined).
ocl_bag_equals(_, oclUndefined, oclUndefined).
...
ocl_bag_unionBag(oclUndefined, _, oclUndefined). 
ocl_bag_unionBag(_, oclUndefined, oclUndefined). 
...
ocl_bag_unionSet(oclUndefined, _, oclUndefined).
ocl_bag_unionSet(_, oclUndefined, oclUndefined).
...
ocl_bag_intersectionBag(_, oclUndefined, oclUndefined).
ocl_bag_intersectionBag(oclUndefined, _, oclUndefined).
...
intersect_with_duplicates(_, [], []) :- !.
intersect_with_duplicates(oclUndefined, _, oclUndefined).
intersect_with_duplicates(_, oclUndefined, oclUndefined).
...
ocl_bag_including(oclUndefined, _, oclUndefined). 
ocl_bag_including(_, oclUndefined, oclUndefined). 
...
ocl_bag_excluding(oclUndefined, _, oclUndefined).
ocl_bag_excluding(_, oclUndefined, oclUndefined).
ocl_bag_excluding([], _, []) :- !.
...
ocl_bag_flatten(oclUndefined, oclUndefined). 
ocl_bag_flatten(Bag, Result) :- 
   flatten(Bag, Result). 
...
ocl_bag_asSet(oclUndefined, oclUndefined). 
ocl_bag_asSet(Bag, Result) :- 
...
ocl_bag_asOrderedSet(oclUndefined, oclUndefined). 
ocl_bag_asOrderedSet(Bag, Result) :- 
   ocl_bag_asSet(Bag, Result).
...
ocl_bag_asSequence(oclUndefined, oclUndefined).
ocl_bag_asSequence(Bag, Result) :- 
...
ocl_bag_asBag(oclUndefined,oclUndefined).
ocl_bag_asBag(Bag, Result) :- 
   ( is_list(Bag) -> Result = Bag; Result = [Bag] ).
...
ocl_seq_equals(oclUndefined, _, oclUndefined). 
ocl_seq_equals(_, oclUndefined, oclUndefined). 
ocl_seq_equals(Seq1, Seq2, Result) :- 
...
ocl_seq_union(oclUndefined, _, oclUndefined). 
ocl_seq_union(_, oclUndefined, oclUndefined). 
ocl_seq_union(Seq1, Seq2, Result) :- 
...
ocl_seq_flatten(oclUndefined, oclUndefined).
ocl_seq_flatten(Seq, Result) :- 
   flatten(Seq, Result). 
...
ocl_seq_append(oclUndefined, _, oclUndefined).
ocl_seq_append(_, oclUndefined, oclUndefined).
ocl_seq_append(Seq, Obj, Result) :- 
...
ocl_seq_prepend(oclUndefined, _,oclUndefined).
ocl_seq_prepend(_, oclUndefined,oclUndefined).
ocl_seq_prepend(Seq, Obj, Result) :- 
   Result = [Obj|Seq].
...
ocl_seq_insertAt(oclUndefined, _, _, oclUndefined).        
ocl_seq_insertAt(_, oclUndefined, _, oclUndefined).        
ocl_seq_insertAt(_, _, oclUndefined, oclUndefined).        
...
ocl_seq_subSequence(oclUndefined, _, _, oclUndefined).
ocl_seq_subSequence(_, oclUndefined, _, oclUndefined).
ocl_seq_subSequence(_, _,oclUndefined, oclUndefined).
...
ocl_seq_indexOf( oclUndefined, _, oclUndefined).
ocl_seq_indexOf( _, oclUndefined, oclUndefined).
...
ocl_seq_first(oclUndefined, oclUndefined).
...
ocl_seq_last(oclUndefined, oclUndefined).
...
ocl_seq_asSet(oclUndefined, oclUndefined). 
ocl_seq_asSet(Seq, Result) :- 
...
ocl_seq_asSequence(oclUndefined, oclUndefined).
ocl_seq_asSequence(Seq, Result) :- 
...
ocl_seq_asBag(oclUndefined, oclUndefined). 
ocl_seq_asBag(Seq, Result) :- 
   ( is_list(Seq) -> Result = Seq; Result = [Seq] ).
...
ocl_seq_asOrderedSet(oclUndefined, oclUndefined).
...
ocl_ordset_append(oclUndefined, _, oclUndefined).
ocl_ordset_append(_, oclUndefined, oclUndefined).
ocl_ordset_append(OrdSet, Obj, Result) :- 
   ocl_mustBeObject(Obj, Object),
...
ocl_ordset_prepend(oclUndefined, _, oclUndefined).
ocl_ordset_prepend(_, oclUndefined, oclUndefined).
ocl_ordset_prepend(OrdSet, Obj, Result) :- 
   ocl_mustBeObject(Obj, Object),
   ( ocl_ordset_includes(OrdSet, Object, 1) ->
     Result = OrdSet;
...
% check if the list is an object list - replacing for nonground(X)
%notObjectList(X) :-
%  X = [_]
%  ->foreach(Obj,X)
%    do
%      type_of(Obj,compound),
%      arg(1,Obj,Oid),
%      ( var(Oid) -> fail;true )
%  ;true.    
