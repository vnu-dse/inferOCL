...
% 2014/02/04:
% - Add ocl_col_isUnique_tuple

% 2014/02/06:
% - Update aux_col_select
%
...
ocl_col_isUnique(Instances, Vars, Collection, Predicate, Result ) :-
  ...
  ( is_oclUndefined(List) -> Result = oclUndefined
  ; Result::0..1,
    list_is_unique(List, Result)
  ).

ocl_col_isUnique_tuple(Instances, Vars, Collection, TuplePredicate, Result ) :-
  apply(TuplePredicate, [_, _, PredList]),
  ( foreach(Obj,Collection),
    fromto([],In,Out,TupleValueList),
    param(Instances,Vars,PredList)
    do
    ( foreach(Predicate,PredList),   
      fromto([],In1,Out1,TupleValue),
      param(Instances,Vars,Obj)
      do
        apply(Predicate,[Instances, [Obj|Vars],Value]),
        Out1 = [Value | In1]
    ),
    Out = [TupleValue | In] 
  ),
...
list_is_unique(X, 1) :- getOidList(X, OidList), list_is_unique(OidList, 1), !.
