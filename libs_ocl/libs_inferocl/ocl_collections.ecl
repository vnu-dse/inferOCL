...
%delay ocl_obj_asSet(X,Y) if nonground(X).
ocl_obj_asSet(X,X) :- is_list(X),!.
ocl_obj_asSet(X,Result) :-
  ocl_obj_asSet_resume(X,Result).

delay ocl_obj_asSet_resume(X,_) if var(X).
ocl_obj_asSet_resume(X,Result) :-
  ( X = [_] -> Result = X
  ; is_oclUndefined(X) -> Result = oclUndefined
  ; Result = [X]
  ).
...
ocl_col_size(Col, Size) :- 
  ( is_list(Col) -> length(Col, Size)
  ; ocl_col_size_resume(Col, Size)
  ).
  
delay ocl_col_size_resume(Col, _) if nonground(Col).
ocl_col_size_resume(Col, Size) :-
  ( is_list(Col) -> length(Col, Size)
  ; Size = oclUndefined
  ).
...
delay ocl_col_includes(X, _, _) if is_obj_list(X), getOidList(X,Y), nonground(Y).
delay ocl_col_includes(_, Y, _) if var(Y).
ocl_col_includes(X, _, oclUndefined) :- is_oclUndefined(X),!.
ocl_col_includes(_, Y, oclUndefined) :- is_oclUndefined(Y),!.
ocl_col_includes(Col, Obj, Result) :- 
   Result::0..1,
   ( foreach(X,Col),
     fromto(0,In,Out,Result),param(Obj) do
...
%ocl_col_includes(Col, Obj, Result) :- 
%   ( is_list(Col) -> 
%      ( Result::0..1; Result = oclUndefined ),
%      ( foreach(X,Col),
%         fromto(0, In, Out, TmpResult),
%         fromto(1, In1, Out1, IsDefined),
%         fromto([], In2, Out2, NongroundList),
%         param(Obj) do
%         ocl_obj_equals(Obj,X,B),
%         ( In #= 1 -> Out #= 1
%         ; ground(B) -> ( B #= 1 -> Out #= 1; Out #= 0 )
%         ; Out #= 0
%         ),
%	 ( is_oclUndefined(B) -> Out1 #= 0; Out1 #= In1 ),
%         ( nonground(X) -> Out2 #= [X|In2]; Out2 #= In2)
%      ),
%      ( TmpResult #= 1 -> Result #= 1
%      ; NongroundList = [] -> ( IsDefined #= 1 -> Result #= 0 ; Result = oclUndefined )  
%      ; ( IsDefined #= 1 -> Col1 #= NongroundList ; Col1 = [oclUndefined | NongroundList] ),
%         ocl_col_includes_resume(Col1, Obj, Result)
%      )
%   ; ocl_col_includes_resume(Col, Obj, Result)
%   )
%delay ocl_col_includes_resume(Col, _, _) if nonground(Col).
%ocl_col_includes_resume(Col, _, oclUndefined) :- is_oclUndefined(Col),!.
%ocl_col_includes_resume(Col, Obj, Result) :- 
%   (Result::0..1;Result=oclUndefined),
%   ( foreach(X,Col),
%      fromto(0, In, Out, TmpResult),
%      fromto(1, InV, OutV, IsDefined),
%      param(Obj) do
%      ocl_obj_equals(Obj,X,B),
%      ( In = 1 -> Out = 1
%        ; B = 1 -> Out = 1
%        ; Out = 0
%      ),
%      ( is_oclUndefined(B) -> OutV = 0; OutV = InV )
%   ),
%   ( TmpResult #= 1 -> Result #= 1
%   ; IsDefined #= 1 -> Result #= 0
%   ; Result = oclUndefined
%   ).
...
delay ocl_col_excludes(X,_,_) if var(X).
delay ocl_col_excludes(X,_,_) if getOidList(X,Y),nonground(Y).
delay ocl_col_excludes(_,X,_) if var(X).
ocl_col_excludes(X,_,oclUndefined) :- is_oclUndefined(X),!.
ocl_col_excludes(_,X,oclUndefined) :- is_oclUndefined(X),!.
ocl_col_excludes(Col, Obj, Result) :-
   ocl_mustBeObject(Obj, Object),
   ocl_col_includes(Col, Object, Aux),
   ocl_col_excludes_resume(Aux,Result).

delay ocl_col_excludes_resume(Aux,_) if nonground(Aux).
ocl_col_excludes_resume(Aux,oclUndefined) :- is_oclUndefined(Aux).
ocl_col_excludes_resume(Aux,Result) :-
   Result::0..1,
   ic:neg(Aux, Result).
...
% TODO: try to remove delay as much as possible

delay ocl_col_count(X, _, _) if nonground(X).
delay ocl_col_count(_, Y, _) if nonground(Y).
ocl_col_count(X,_,oclUndefined) :- is_oclUndefined(X),!.
ocl_col_count(_,X,oclUndefined) :- is_oclUndefined(X),!.
ocl_col_count( Col, Obj, Result ) :-
  ocl_mustBeObject(Obj, Object),
       ( 
         Result::0..1,
...
%    and 0 otherwise
% TODO: check if Col2 contains oclUndefined ...

delay ocl_col_includesAll(X, _, _) if nonground(X).
delay ocl_col_includesAll(_, Y, _) if nonground(Y).
ocl_col_includesAll(X,_,oclUndefined) :- is_oclUndefined(X),!.
ocl_col_includesAll(_,X,oclUndefined) :- is_oclUndefined(X),!.
ocl_col_includesAll(X,_,oclUndefined) :- is_undefined_list(X),!.
ocl_col_includesAll(_,X,oclUndefined) :- is_undefined_list(X),!.
ocl_col_includesAll(Col1, Col2, Result) :-
       (   
         Col2 = [] 
         -> Result = 1; 
...
delay ocl_col_excludesAll(_, X, _) if nonground(X).
ocl_col_excludesAll(X, _, oclUndefined)  :- is_oclUndefined(X),!.
ocl_col_excludesAll([], _, 1) :- !.
ocl_col_excludesAll(_, [], 1) :- !.
ocl_col_excludesAll(_, X, oclUndefined)  :- is_oclUndefined(X),!.
%ocl_col_excludesAll(X, _, oclUndefined)  :- is_undefined_list(X),!.
%ocl_col_excludesAll(_, X, oclUndefined)  :- is_undefined_list(X),!.
ocl_col_excludesAll(Col1, Col2, Result) :-
...
delay ocl_col_isEmpty(X, _) if nonground(X).
ocl_col_isEmpty(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_col_isEmpty([], 1) :- !.
...
delay ocl_col_notEmpty(X, _) if nonground(X).
ocl_col_notEmpty(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_col_notEmpty([], 0) :- !.
...
delay ocl_col_sum(X, _) if nonground(X).
ocl_col_sum(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_col_sum(Col, Result) :- 
  is_undefined_list(Col)
  -> Result = oclUndefined;
  Result #= sum(Col).
...
delay ocl_col_product(X, Y, _) if nonground([X,Y]).
ocl_col_product(X, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_col_product(_, Y, oclUndefined) :- is_oclUndefined(Y), !.
ocl_col_product([], _, []) :- !.
ocl_col_product(_, [], []) :- !.
...
% TODO: oclUndefined
cartesian_product(X, _, oclUndefined) :- is_oclUndefined(X), !.
cartesian_product(_, Y, oclUndefined) :- is_oclUndefined(Y), !.
...
ocl_set_including(X, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_set_including(_, X, oclUndefined) :- is_oclUndefined(X), !.
ocl_set_including(Set, Obj, Result) :-  
   ocl_mustBeObject(Obj, Object),
...
ocl_set_unionSet(X, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_set_unionSet(_, Y, oclUndefined) :- is_oclUndefined(Y), !.
...
ocl_set_unionBag(X, _, oclUndefined) :- is_oclUndefined(X), !. 
ocl_set_unionBag(_, Y, oclUndefined) :- is_oclUndefined(Y), !. 
...
ocl_set_equals(X, _, oclUndefined) :- is_oclUndefined(X), !. 
ocl_set_equals(_, Y, oclUndefined) :- is_oclUndefined(Y), !. 
ocl_set_equals(X, _, oclUndefined) :- is_undefined_list(X), !. 
ocl_set_equals(_, Y, oclUndefined) :- is_undefined_list(Y), !. 
ocl_set_equals(Set1, Set2, Result) :- 
   list_to_ord_set(Set1, Aux1),
   list_to_ord_set(Set2, Aux2),
...
ocl_set_intersectionSet(X, _, oclUndefined) :- is_oclUndefined(X), !. 
ocl_set_intersectionSet(_, Y, oclUndefined) :- is_oclUndefined(Y), !. 
ocl_set_intersectionSet(X, _, oclUndefined) :- is_undefined_list(X), !.
ocl_set_intersectionSet(_, Y, oclUndefined) :- is_undefined_list(Y), !. 
...
ocl_set_intersectionBag(X, _, oclUndefined) :- is_oclUndefined(X), !. 
ocl_set_intersectionBag(_, Y, oclUndefined) :- is_oclUndefined(Y), !. 
ocl_set_intersectionBag(X, _, oclUndefined) :- is_undefined_list(X), !. 
ocl_set_intersectionBag(_, Y, oclUndefined) :- is_undefined_list(Y), !. 
...
ocl_set_difference(X, _, oclUndefined) :- is_oclUndefined(X), !. 
ocl_set_difference(_, Y, oclUndefined) :- is_oclUndefined(Y), !. 
ocl_set_difference(X, _, oclUndefined) :- is_undefined_list(X), !. 
ocl_set_difference(_, Y, oclUndefined) :- is_undefined_list(Y), !. 
ocl_set_difference(Set1, Set2, Result):- 
   list_to_ord_set(Set1, Aux1),
   list_to_ord_set(Set2, Aux2),
...
ocl_set_excluding(X, _, oclUndefined ) :- is_oclUndefined(X), !.
ocl_set_excluding(_, Y, oclUndefined ) :- is_oclUndefined(Y), !.
ocl_set_excluding(X, _, oclUndefined ) :- is_undefined_list(X), !.
ocl_set_excluding(Set, Obj, Result ):- 
   ocl_mustBeObject(Obj, Object),
   list_to_ord_set(Set, Aux),
...
ocl_set_symmetricDifference(X, _, oclUndefined ) :- is_oclUndefined(X), !.
ocl_set_symmetricDifference(_, Y, oclUndefined ) :- is_oclUndefined(Y), !.
ocl_set_symmetricDifference(X, _, oclUndefined ) :- is_undefined_list(X), !.
ocl_set_symmetricDifference(_, Y, oclUndefined ) :- is_undefined_list(Y), !.
ocl_set_symmetricDifference(Set1, Set2, Result) :-
   list_to_ord_set(Set1, Aux1),
   list_to_ord_set(Set2, Aux2),
...
delay ocl_set_flatten(X, _) if nonground(X).
ocl_set_flatten(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_set_flatten(Set, Result) :- 
   flatten(Set, Result).
...
delay ocl_set_asSet(X, _) if nonground(X).
ocl_set_asSet(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_set_asSet(Set, Result) :- 
...
ocl_set_asOrderedSet(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_set_asOrderedSet(Set, Result) :- 
...
delay ocl_set_asSequence(X, _) if nonground(X).
ocl_set_asSequence(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_set_asSequence(Set, Result) :- 
   ( is_list(Set) -> Result = Set; Result = [Set]).
...
delay ocl_set_asBag(X, _) if nonground(X).
ocl_set_asBag(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_set_asBag(Set, Result) :- 
   ( is_list(Set) -> Result = Set; Result = [Set]).
...
ocl_bag_equals(X, _, oclUndefined ) :- is_oclUndefined(X), !.
ocl_bag_equals(_, Y, oclUndefined ) :- is_oclUndefined(Y), !.
ocl_bag_equals([], [], 1):- !.
ocl_bag_equals(X, _, oclUndefined) :- is_undefined_list(X), !.
ocl_bag_equals(_, Y, oclUndefined) :- is_undefined_list(Y), !.
ocl_bag_equals([First1|Rest1],[First2|Rest2],Result) :-
...
ocl_bag_unionBag(X, _, oclUndefined) :- is_oclUndefined(X), !. 
ocl_bag_unionBag(_, Y, oclUndefined) :- is_oclUndefined(Y), !. 
...
ocl_bag_unionSet(X, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_bag_unionSet(_, Y, oclUndefined) :- is_oclUndefined(Y), !.
...
ocl_bag_intersectionBag(X, _, oclUndefined ) :- is_oclUndefined(X), !.
ocl_bag_intersectionBag(_, Y, oclUndefined ) :- is_oclUndefined(Y), !.
ocl_bag_intersectionBag(_, Y, oclUndefined) :- is_undefined_list(Y), !.
ocl_bag_intersectionBag(X, _, oclUndefined) :- is_undefined_list(X), !.
...
delay intersect_with_duplicates(X,Y,_) if nonground([X,Y]).
intersect_with_duplicates(X, _, oclUndefined ) :- is_oclUndefined(X), !.
intersect_with_duplicates(_, Y, oclUndefined ) :- is_oclUndefined(Y), !.
intersect_with_duplicates([], _, []) :- !.
intersect_with_duplicates(_, [], []) :- !.
intersect_with_duplicates(X, _, oclUndefined) :- is_undefined_list(X), !.
intersect_with_duplicates(_, Y, oclUndefined) :- is_undefined_list(Y), !.
...
delay ocl_bag_intersectionSet(Bag, Set, _) if nonground([Bag,Set]).
...
delay ocl_bag_including(X, Y, _) if nonground([X,Y]).
ocl_bag_including(X, _, oclUndefined) :- is_oclUndefined(X), !. 
%ocl_bag_including(_, Y, oclUndefined) :- is_oclUndefined(Y), !. 
...
ocl_bag_excluding(X, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_bag_excluding(_, Y, oclUndefined) :- is_oclUndefined(Y), !.
ocl_bag_excluding(X, _, oclUndefined) :- is_undefined_list(X), !.
ocl_bag_excluding([], _, []) :- !.
...
ocl_bag_flatten(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_bag_flatten(Bag, Result) :- 
   flatten(Bag, Result). 
...
delay ocl_bag_asSet(X, _) if var(X).
ocl_bag_asSet(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_bag_asSet(Bag, Result) :- 
...
delay ocl_bag_asOrderedSet(Bag, _) if nonground(Bag).
ocl_bag_asOrderedSet(Bag, Result) :- 
   ocl_bag_asSet(Bag, Result).
...
delay ocl_bag_asSequence(X, _) if nonground(X).
ocl_bag_asSequence(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_bag_asSequence(Bag, Result) :- 
...
delay ocl_bag_asBag(X, _) if nonground(X).
ocl_bag_asBag(X,oclUndefined) :- is_oclUndefined(X), !.
ocl_bag_asBag(Bag, Result) :- 
   ( is_list(Bag) -> Result = Bag; Result = [Bag] ).
...
ocl_seq_equals(X, _, oclUndefined) :- is_undefined_list(X), !. 
ocl_seq_equals(_, Y, oclUndefined) :- is_undefined_list(Y), !. 
ocl_seq_equals(Seq1, Seq2, Result) :- 
...
delay ocl_seq_union(X, _, _) if nonground(X).
delay ocl_seq_union(_, Y, _) if nonground(Y).
ocl_seq_union(X, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_seq_union(_, Y, oclUndefined) :- is_oclUndefined(Y), !. 
ocl_seq_union(Seq1, Seq2, Result) :- 
...
ocl_seq_flatten(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_seq_flatten(Seq, Result) :- 
   flatten(Seq, Result). 
...
delay ocl_seq_append(X, _, _) if nonground(X).
ocl_seq_append(X, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_seq_append(_, Y, oclUndefined) :- is_oclUndefined(Y), !.
ocl_seq_append(Seq, Obj, Result) :- 
...
delay ocl_seq_prepend(X, _, _) if nonground(X).
ocl_seq_prepend(X, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_seq_prepend(_, Obj, oclUndefined) :- is_oclUndefined(Obj), !.
ocl_seq_prepend(Seq, Obj, Result) :- 
   Result = [Obj|Seq].
...
delay ocl_seq_insertAt(X, _, _, _) if nonground(X).
delay ocl_seq_insertAt(_, _, Y, _) if nonground(Y).
ocl_seq_insertAt(X, _, _, oclUndefined) :- is_oclUndefined(X), !. 
ocl_seq_insertAt(_, Obj, _, oclUndefined) :- is_oclUndefined(Obj), !.         
ocl_seq_insertAt(_, _, Y, oclUndefined) :- is_oclUndefined(Y), !.        
...
delay ocl_seq_subSequence(X, _, _, _) if nonground(X).
delay ocl_seq_subSequence(_, Y, _, _) if nonground(Y).
delay ocl_seq_subSequence(_, _, Z, _) if nonground(Z).
ocl_seq_subSequence(X, _, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_seq_subSequence(_, Y, _, oclUndefined) :- is_oclUndefined(Y), !.
ocl_seq_subSequence(_, _, Z, oclUndefined) :- is_oclUndefined(Z), !.
...
delay ocl_seq_at(X, _, _) if nonground(X).
delay ocl_seq_at(_, Y, _) if nonground(Y).
ocl_seq_at( X, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_seq_at( _, Y, oclUndefined) :- is_oclUndefined(Y), !.
...
ocl_seq_indexOf( X, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_seq_indexOf( _, Y, oclUndefined) :- is_oclUndefined(Y), !.
ocl_seq_indexOf( [], _, oclUndefined) :- !.
...
ocl_seq_indexOf( [First|Rest], Obj, Result) :- 
       ...
       ( N = oclUndefined -> Result = oclUndefined; Result is N + 1).
                
% ocl_seq_first( +Seq, -Result ) :- 
%    Return the first element in the sequence SEQ
%    Behavior is not specified if the sequence is empty

delay ocl_seq_first(X, _) if nonground(X).
ocl_seq_first(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_seq_first([], oclUndefined).
ocl_seq_first([X|_], X) :- !.
...
delay ocl_seq_last(X, _) if nonground(X).
ocl_seq_last([], oclUndefined) :- !.
ocl_seq_last(X, oclUndefined) :- is_oclUndefined(X), !.
...
ocl_seq_including(X, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_seq_including(_, X, oclUndefined) :- is_oclUndefined(X), !.
ocl_seq_including(Seq, Obj, Result) :- 
   ocl_mustBeObject(Obj, Object),
...
ocl_seq_excluding(X, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_seq_excluding(_, X, oclUndefined) :- is_oclUndefined(X), !.
ocl_seq_excluding(Seq, Obj, Result) :- 
   ocl_mustBeObject(Obj, Object),
...
ocl_seq_asSet(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_seq_asSet(Seq, Result) :- 
...
delay ocl_seq_asSequence(X, _) if nonground(X).
ocl_seq_asSequence(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_seq_asSequence(Seq, Result) :- 
...
delay ocl_seq_asBag(X, _) if nonground(X).
ocl_seq_asBag(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_seq_asBag(Seq, Result) :- 
   ( is_list(Seq) -> Result = Seq; Result = [Seq] ).
...
delay ocl_seq_asOrderedSet(X, _) if var(X).
ocl_seq_asOrderedSet(X, oclUndefined) :- is_oclUndefined(X), !.
ocl_seq_asOrderedSet([], []) :- !.
ocl_seq_asOrderedSet(X, [X]) :- is_not_list(X), !.
...
ocl_ordset_append(X, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_ordset_append(_, Y, oclUndefined) :- is_oclUndefined(Y), !.
ocl_ordset_append(OrdSet, Obj, Result) :- 
   ocl_mustBeObject(Obj, Object),
...
ocl_ordset_prepend(X, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_ordset_prepend(_, Y, oclUndefined) :- is_oclUndefined(Y), !.
ocl_ordset_prepend(OrdSet, Obj, Result) :- 
   ocl_mustBeObject(Obj, Object),
   ( ocl_ordset_includes(OrdSet, Object, 1) ->
     Result = OrdSet;
...
ocl_ordset_insertAt(X, _, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_ordset_insertAt(_, X, _, oclUndefined) :- is_oclUndefined(X), !.
ocl_ordset_insertAt(_, _, X, oclUndefined) :- is_oclUndefined(X), !.
ocl_ordset_insertAt(OrdSet, Obj, Idx, Result) :-
   ocl_mustBeObject(Obj, Object),
   ( ocl_ordset_includes(OrdSet, Object, 1) 
     -> Result = OrdSet;
...
% check if a list contains an oclUndefined
is_undefined_list(List) :-
   ground(List),
   ( foreach(X,List),
     fromto(0, In, Out, Ret)
     do	
        ( is_oclUndefined(X) -> Out = 1; Out = In )
   ),
   Ret = 1.
% chec if it is not a list
is_not_list(X) :-
  ( var(X)
  ; X \= [_]
  ).
% chec if it is a list of objects
is_obj_list(List) :-
  is_list(List),
  ( foreach(X,List)
    do
