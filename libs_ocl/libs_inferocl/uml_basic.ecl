...
getOidList(ObjectList, OidList) :-
  ( is_oclUndefined(ObjectList)
    -> OidList = oclUndefined;
    is_list(ObjectList),	
...
existsOidInList(Oid, OidList) :-
   ...
   ( foreach(Other, OidList),
     ...
     param(Oid)
     do 
       ( is_oclUndefined(Other) -> Aux = 0; #=(Oid, Other, Aux) ),
       ic:or(In, Aux, Out)
   ),
   Result #= 1.
...
countOidInList( X, _, oclUndefined) :- is_oclUndefined(X),!.
countOidInList( _, X, oclUndefined) :- is_oclUndefined(X),!.
countOidInList( OidList, Oid, Result ) :-
   ( foreach(Elem, OidList),
     ...
     do 
       ...
       ( is_oclUndefined(Elem) -> TruthValue = 0; #=(Elem, Oid, TruthValue) )
   ),
   Result #= sum(Found).
...
differentOids(Instances):-
   ...
   ( is_undefined_list(OidList) -> fail;
...
orderForAttributes(ObjectA, ObjectB) :-
  ObjectA \= oclUndefined,
  ObjectB \= oclUndefined,
...
disjointOids( TypeList ) :-
	...
	( is_undefined_list(AllOids) -> fail; ic_global:alldifferent( AllOids )).       
...
nth1( _, [], _ ) :- 
   ...
   %abort. 
   writeln("Warning: Term nth1/3 invoked with empty list"),  
   fail.
...
ocl_attributeCall2( _, _, _, Oid, oclUndefined ) :- is_oclUndefined(Oid),!.
ocl_attributeCall2( Instances, TypeName, AttribName, Oid, Result ) :-
