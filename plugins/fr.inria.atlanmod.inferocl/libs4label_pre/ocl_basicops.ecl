...
ocl_and(Instances, Vars, Pred1, Pred2, Result) :-
   ...
   ( TruthValue1 = oclUndefined
     -> Result = oclUndefined;
     ( TruthValue2 = oclUndefined
       -> Result = oclUndefined;
       (    
         Result::0..1,
...
ocl_obj_equals(_, oclUndefined, oclUndefined):- !.
ocl_obj_equals(oclUndefined, _, oclUndefined):- !.
...
delay ocl_attributeCall(_, _, _, X, _) if var(X).
ocl_attributeCall(_, _, _, oclUndefined, oclUndefined)  :- !.
...
ocl_attributeCall_noAssocCls(_, _, _, oclUndefined, oclUndefined)  :- !.
ocl_attributeCall_noAssocCls(Instances, TypeName, AttribName, Obj, Result)  :-
  ocl_mustBeObject(Obj, Object),
  ( attIndex(TypeName, AttribName, Index) 
    ->
...
ocl_navigation(_, _, _, _, oclUndefined, oclUndefined) :- !.
...
ocl_mustBeObject(oclUndefined, oclUndefined):- !.
